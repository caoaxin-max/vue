  ...
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="100px" v-loading="formLoading"  :rules="rules">
      ...
      <el-form-item label="题干：" prop="title" required>
        <el-input v-model="form.title" placeholder="请在最开始加上：'填空：'" @focus="inputClick(form,'title')" />
      </el-form-item>
    ...
    <el-dialog @open="openEvent" :visible.sync="richEditor.dialogVisible"  append-to-body :close-on-click-modal="false" style="width: 100%;height: 100%"   :show-close="false" center>
      <el-row v-loading="!richEditor.isShow">
        <Ueditor ref="ueditor" v-show="richEditor.isShow"/>
      </el-row>
...
export default {
  ...
  created () {
    ...
    this.initSubject(function () {
      _this.subjectFilter = _this.subjects.filter(data => data.level === level)
    })
    if (id && parseInt(id) !== 0) {
  ...
  methods: {
    openEvent(){
      setTimeout(()=>{
        this.richEditor.isShow = true
        this.richEditor.editor = this.$refs.ueditor.editor
        this.editorReady(this.richEditor.editor)
      }, 500)
    },
    editorReady (instance) {
      const editor = instance
      let currentContent = this.richEditor.object[this.richEditor.parameterName]
      editor.setHtml(currentContent)
      // 光标定位到Ueditor
      editor.focus(true)
    },
    ...
    editorConfirm () {
      let content = this.richEditor.editor.getText()
